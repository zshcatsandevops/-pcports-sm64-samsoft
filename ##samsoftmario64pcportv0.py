#!/usr/bin/env python3
# MARIO-STYLE 3D PLATFORMER - Peach's Castle Tech Demo
from ursina import *
from ursina.prefabs.first_person_controller import FirstPersonController
import random

app = Ursina()
window.title = "Mario-style 3D Platformer - Peach's Castle"
window.size = (800, 600)
...
def create_castle():
    ...
    for pos in tower_positions:
        tower = Entity(model='cylinder', scale=(3, 10, 3), position=pos, color=color.pink, collider='box')
...
# UI Elements
coins_text = Text(text=f"Coins: {coins_collected}/{total_coins}", position=(-0.8, 0.45), scale=2)
lives_text = Text(text=f"Lives: {lives}", position=(-0.8, 0.4), scale=2)
score_text = Text(text=f"Score: {player_score}", position=(-0.8, 0.35), scale=2)
message_text = Text(text="Collect all coins and reach the crown!", position=(-0.7, 0.3), scale=2)
...
def input(key):
    if key == 'r':
        global coins_collected, lives, player_score, state, coins
        state = PLAYING
        coins_collected = 0
        lives = 3
        player_score = 0
